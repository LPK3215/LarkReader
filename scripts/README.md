...
| `npm run release -- <版本>` | `scripts/release.mjs` | 一键发版：跑门禁 → 打 `v<版本>` tag → 推 main + tag → 触发 GitHub Actions 三平台出包 |
...
cd src-tauri
cargo test --test z_tmp_full_download
...
- 发版走 `npm run release`：GitHub Actions 的 workflow（`.github/workflows/publish.yml`）监听 `v*` tag，产物是 draft release，去 Releases 页确认后点发布。
