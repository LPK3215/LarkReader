...
当前后端支持环境与登录检测、单文档 Markdown/图片导出、Wiki 目录递归导出、Sheet XLSX、Bitable NDJSON、后台任务进度与取消、任务历史、结构化错误和安全文件保存。
...
- **测试数据**：`e2e-fixtures/` 保存了 Sheet（`sheet_cells.json`）与 Bitable（`bitable_records.json`）的写入数据，以及媒体素材与生成脚本（`gen_assets.py`）。
