...
> **状态更新（2026-09-05）**：本文记录的 O-01 ～ O-07、N-01 ～ N-08 已全部修复。本文保留为历史审查证据，不再代表当前待办。
>
...
> - N-02/N-03：完成结果进入有界完成表，可通过 `get_task_result` 领取；活动任务结束即移除。
> - N-04：启动任务前先读取 Wiki 树并计算真实总数。
> - N-05：取消信号进入图片下载和子进程等待循环，取消时终止当前子进程。
> - N-06：知识库目录通过原子 `create_dir` 竞争创建，不再使用 `exists + create`。
> - N-07：图片 URL 使用 Markdown 源码 offset 精确替换。
> - N-08：图片下载使用受 `Settings.concurrency` 控制的 Rayon 线程池。
