...
- **仅导出本节点及子树**（Auto，默认）
  适合目标 URL 本身是一个带子文档的目录/父文档：只导出它及后代。
- **展开整个知识库（含兄弟节点）**（FullSpace）
  适合把 URL 贴到“首页/入口页”——这种页面在飞书里通常**没有子节点**，
  FullSpace 会自动列出该知识库空间下的全部顶层节点并递归展开，一次拿到整库。
...
2. 左侧是目录树（懒加载、目录优先排序），点击 `.md` 在右侧渲染正文
3. 图片会自动按相对路径读回本地并内联显示；`.md` 之外的附件只作展示不可读
...
| 扫描只看到 1 个节点（入口页） | 这是页面本身没有子文档；改用「展开整个知识库」模式重扫 |
